  ...
  return (
    ...
    >
      ...
      <Section title="5. アカウント情報">
        ...
        <ul className="list-disc space-y-1 pl-5">
          <li>メールアドレス</li>
          <li>Google アカウントでログインした場合、Google から提供される基本プロフィール(氏名・メールアドレス)</li>
          <li>登録日時・最終ログイン日時</li>
        </ul>
        <p>
      ...
      <Section title="6. 閲覧記録">
        ...
        <p>
          <strong>IPアドレスおよびブラウザの種類(ユーザーエージェント)は取得していません。</strong>
          ブラウザごとの識別子は当サイト内の閲覧をまとめるためだけに用い、個人を特定する目的では使用しません。
          ブラウザの保存データを消去すると、この識別子も削除されます。
        </p>
        <p>
          現時点で外部のアクセス解析サービス(Google Analytics など)および広告目的の Cookie は使用していません。
          導入する場合は本ポリシーを改定し、その旨を記載します。
        </p>
      </Section>
